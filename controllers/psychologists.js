...
const getById = (req, res) => {
  ...
  if (!psyFound) {
    return res
      .status(404)
      .json({ message: `Psychologist not found with id: ${id}` });
  }
  res.json(psyFound);
};
...
const getByName = (req, res) => {
  const psyFound = psyList.filter(
    (psy) =>
      psy.firstName === req.query.firstName &&
      psy.lastName === req.query.lastName
  );
...
const add = (req, res) => {
  ...
  psyList.push(newPsy);
  fs.writeFile(
    path.join(__dirname, "../data/psychologists.json"),
...
const edit = (req, res) => {
  ...
  const psyFound = psyList.find((psy) => psy.id === id);
  if (!psyFound) {
    return res.status(404).json({ message: `psy not found with id ${id}` });
  }
  ...
  fs.writeFile(
    ...
    (err) => {
      if (err) {
        console.log(err);
        return res.status(500).json({ message: "Error edting psychologist" });
      }
