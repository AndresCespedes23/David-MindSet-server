...
const getByName = (req, res) => {
  ...
  const foundCandidates = candidates.list.filter((candidate) =>
  candidate.firstName.toLowerCase() === name.toLowerCase());
...
const add = (req, res) => {
  const lastID = getLastId(candidates.list);
  const newCandidate = {
    id: lastID + 1,
...
const edit = (req, res) => {
  ...
  candidates = candidates.list.map((candidate) => {
    if (candidate.id === candidateID) {
      ...
      candidate.education =
      req.query.education !== undefined ? [req.query.education] : candidate.education;
      candidate.experiences =
        req.query.experiences !== undefined ? [req.query.experiences] : candidate.experiences;
      ...
      req.query.mainSkills !== undefined ? [req.query.mainSkills] : candidate.mainSkills;
      candidate.profileTypes =
      req.query.profileTypes !== undefined ? [req.query.profileTypes] : candidate.profileTypes;
      candidate.isOpenToWork = req.query.isOpenToWork || candidate.isOpenToWork;
      updatedCandidate = candidate;
    }
    return candidate;
  });
  fs.writeFile(path.join(__dirname, '../data/candidates.json'),
   JSON.stringify({ list: candidates }), (err) => {
    if (err) {
      return res.status(500).json({ message: 'Error while saving data' });
    }
    res.json({
